test_that("IS works when using the prior as proposal, and when using a custom proposal", {
  ...
  expect_equal(signif(test_is_prior_as_proposal_var(), digits = 2), signif(1/(1+1), digits = 2))
  ...
  expect_equal(signif(test_is_custom_proposal_var(), digits = 2), signif(1/(1+1), digits = 2))
